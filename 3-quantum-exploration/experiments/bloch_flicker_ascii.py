...
import random
import math
import shutil  # <-- NEW: for terminal size
...
def run_bloch_ascii_sphere(prob_one: float = 0.5, frames: int = 250, delay: float = 0.05):
    ...
    Resizes with the terminal: width/height are recomputed every frame.
    """
    count_zero = 0
    count_one = 0
    total = 0
    ...
    for step in range(frames):
        # --- 1) Terminal size & layout ---
        term_size = shutil.get_terminal_size(fallback=(80, 24))
        full_width = term_size.columns
        full_height = term_size.lines

        # Reserve a few lines for text above/below the sphere
        header_lines = 5
        footer_lines = 6
        height = max(10, full_height - header_lines - footer_lines)
        width = max(30, full_width - 4)

        # --- 2) Simulate one measurement ---
        ...
        # --- 3) Build sphere outline for CURRENT size ---
        sphere_points = []
        ...
        # --- 4) Build empty screen for this frame ---
        ...
        # --- 5) Arrow tip positions in x–z plane ---
        x_tip = 0.0
        ...
        lines.append("   top ≈ |0> (z=+1), bottom ≈ |1> (z=-1)")
        ...
        # Move cursor to top-left and redraw scene in-place
...
if __name__ == "__main__":
    run_bloch_ascii_sphere(prob_one=0.5, frames=50, delay=0.05)